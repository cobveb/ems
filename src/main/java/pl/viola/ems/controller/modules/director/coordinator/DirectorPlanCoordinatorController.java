...
public class DirectorPlanCoordinatorController {
    ...
    public ApiResponse getPlansByDirector(@PathVariable int year) {
        return new ApiResponse(HttpStatus.FOUND, planService.getPlansCoordinatorInDirector(false, year));
    }

    @GetMapping("/plans/{year}/getCoordinatorsPlanUpdates")
    @PreAuthorize("hasGroup('admin') or hasPrivilege('1015')")
    public ApiResponse getCoordinatorsPlanUpdates(@PathVariable int year) {
        return new ApiResponse(HttpStatus.FOUND, planService.getPlansCoordinatorInDirector(true, year));
    }
